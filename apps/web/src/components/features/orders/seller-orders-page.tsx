...
import { Package, Inbox, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function SellerOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const result = await listSellerOrders(params);
  ...
  const updateParams = (key: string, value: string) => {
    ...
    router.push(`/${locale}/seller/orders?${params.toString()}`);
  };

  // Count items needing action (PROCESSING status = needs shipping)
  const getProcessingCount = (order: Order): number => {
    return order.orderItems.filter((item) => item.status === 'PROCESSING').length;
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-4">
          ...
          <p className="text-lg text-muted-foreground">{t('noSellerOrders')}</p>
        </div>
      ) : (
        /* Order list */
        <div className="space-y-3">
          {orders.map((order) => {
            const processingCount = getProcessingCount(order);
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {tOrders('date')}: {formatDate(order.createdAt)} &middot;{' '}
                            {t('yourItems')}: {order.orderItems.length}
                          </p>
                        </div>
                      </div>

                      {/* Right: status badges + total */}
                      <div className="flex items-center gap-2 flex-wrap sm:flex-col sm:items-end">
                        <div className="flex items-center gap-2">
                          ...
                          {processingCount > 0 && (
                            ...
                            >
                              {processingCount} {t('pendingShipment')}
                            </Badge>
                          )}
                        </div>
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="flex items-center justify-center gap-4 pt-4">
          ...
          <span className="text-sm text-muted-foreground">
            {tOrders('pageOf', { current: currentPage, total: pagination.totalPages })}
          </span>
          <Button
            variant="outline"
            size="sm"
