// Import Open Library data into Comics Trunk production database
// Run from: /home/ferna5257/applications/api.comicstrunk.com/
// Usage: node import-openlibrary-prod.js /home/ferna5257/import-ready.json
...
function extractEditionNumber(title) {
  const patterns = [/#\s*(\d+)/, /Vol\.?\s*(\d+)/i, /Volume\s*(\d+)/i, /Book\s*(\d+)/i];
...
async function run() {
  ...
  if (!inputFile || !fs.existsSync(inputFile)) {
    console.log('Usage: node import-openlibrary-prod.js <input-json>');
    process.exit(1);
  }

  const data = JSON.parse(fs.readFileSync(inputFile, 'utf8'));
  console.log('=== OPEN LIBRARY IMPORT (PRODUCTION) ===');
  console.log('Input: ' + inputFile);
  console.log('Entries in file: ' + data.length);

  // Step 1: Check existing sourceKeys
  console.log('\nChecking existing entries by sourceKey...');
  ...
  while (true) {
    ...
    page++;
    if (page % 10 === 0) process.stdout.write('  page ' + page + '\r');
  }
  ...
  for (const group of seriesGroups) {
    ...
    try {
      const series = await prisma.series.create({
        data: { title: group.name, slug: finalSlug, totalEditions: group.entries.length },
      });
      seriesIdMap.set(group.name + '||' + group.publisher, series.id);
      seriesCreated++;
    } catch (e) {
      console.log('  Series fail: ' + group.name + ' - ' + e.message.slice(0, 60));
    }
    if (seriesCreated % 100 === 0) process.stdout.write('  Series: ' + seriesCreated + '\r');
  }
  ...
  console.log('Existing slugs loaded: ' + existingEntrySlugs.size);

  const allEntries = [];
  for (const group of seriesGroups) {
    const seriesId = seriesIdMap.get(group.name + '||' + group.publisher) || null;
    for (const entry of group.entries) allEntries.push({ ...entry, seriesId });
  }
  for (const entry of standalone) allEntries.push({ ...entry, seriesId: null });
  ...
  for (let i = 0; i < allEntries.length; i += BATCH_SIZE) {
    ...
    for (const entry of batch) {
      ...
      batchData.push({
        ...
        coverFileName: entry.coverFileName || null,
        approvalStatus: 'APPROVED',
        averageRating: 0,
        ratingCount: 0,
        createdById: ADMIN_ID,
        seriesId: entry.seriesId || null,
        editionNumber: extractEditionNumber(entry.title),
      });
    }

    if (batchData.length > 0) {
      try {
        await prisma.catalogEntry.createMany({ data: batchData, skipDuplicates: true });
