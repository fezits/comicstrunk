...
import { createCategory, updateCategory, deleteCategory } from '@/lib/api/admin-catalog';
...
export default function AdminCategoriesPage() {
  ...
  const openCreate = () => {
    setEditItem(null); setName(''); setDescription(''); setDialogOpen(true);
  };

  const openEdit = (item: CategoryItem) => {
    setEditItem(item); setName(item.name); setDescription(item.description ?? ''); setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await updateCategory(editItem.id, { name, description: description || undefined });
      } else {
